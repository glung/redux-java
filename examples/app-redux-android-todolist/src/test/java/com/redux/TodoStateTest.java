...
public class TodoStateTest {
    ...
    public void createFromShouldOrderTodoByAscendantId() {
        ...
        assertThat(TodoState.from(Arrays.asList(todo1, todo2, todo0))
                .getTodoList())
    ...
    public void addShouldPreserveThePreviousVersion() {
        ...
        assertThat(state.add(todo).getTodoList()).containsExactly(todo);
        assertThat(state.getTodoList()).isEmpty();
    }

    @Test
    public void removeShouldPreserveThePreviousVersion() {
        final Todo todo = new Todo(0, "test", false);
        state = TodoState.from(todo);

        assertThat(state.remove(todo).getTodoList()).isEmpty();
        assertThat(state.getTodoList()).containsExactly(todo);
    }
    ...
    public void getTodoListShouldReturnUnmodifiableList() {
        state.getTodoList().add(new Todo(0, "test", true));
    }
